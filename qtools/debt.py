...
class Debt:
    ...
    def compute_fed(self, index_code: str, compute_method: str):
        ...
        df_debt = self.debt_obj.load_national_debt_data(area_code="cn", rows=["date", "tcm_y10"])
        df = pd.merge(df_index, df_debt, on="date", how="inner")
        if compute_method == "-": 
            df["fed"] = round((1/df["pe_ttm"] - df["tcm_y10"]) * 100, 3)
        else:
            df["fed"] = round((1/df["pe_ttm"] / df["tcm_y10"]), 3)
